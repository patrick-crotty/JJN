#!/usr/bin/env python3
# jjn.py -- functions to calculate the action potential from a
...
# STO with sinusoidal bias current:
...
# STO with constant bias current:
izh_param['STO'] = (JJN(param=JJN_Param(1.71, 0.6494499, 0.129869,0.49, 0.48),
                        i_bias=I_Constant(1.9),
                        i_in=I_Pulse(1.12739, 4.6479, 851.82)), 2000)
...
def int_jj1(jjn, t_end=DEFAULT_T_END, num_t=20001, atol=DEFAULT_ATOL,
    ...
    for j in range(len(t)):
        f[j] = jjn.p.Lambda * (s[0, j] + s[2, j])
        s_f[j] = (s[0, j], s[1, j], s[2, j], s[3, j], f[j])
...
def mess_around(jjn=izh_param['TS'][0], index=1):
    '''Graphical tool to explore JJN parameter space.
    inputs:  jjn:  a JJN object.  (Default/initial:  TS)
    ...
    def display_help_window(vald):
        ...
        helpwin.text(0.05, 0.55, 'TS, PS, ..., TV:  Izhikevich behaviors',
                     fontsize=12)
        helpwin.text(0.1, 0.5, '(compare with '
                     'https://www.izhikevich.org/publications/figure1.pdf )',
                     fontsize=12)
        helpwin.text(0.05, 0.45, '$\phi_p$, $\dot{\phi}_p$, $\phi_c$, '
                     '$\dot{\phi}_c$:  junction phase or derivative to plot',
                     fontsize=12)
        helpwin.text(0.05, 0.4, '$\Phi$:  plot magnetic flux '
                     '($\Phi = \Lambda (\phi_p + \phi_c)$)', fontsize=12)
        helpwin.text(0.05, 0.35, 'W:  write data on plot to jj\_ap.out',
                     fontsize=12)
        helpwin.text(0.05, 0.3, '!:  recenter sliders', fontsize=12)
        helpwin.text(0.05, 0.25, '?:  print current parameter values to '
                     'terminal', fontsize=12)
        helpwin.text(0.05, 0.2, 'Help:  this window', fontsize=12)
        helpwin.text(0.05, 0.05, 'See Crotty, Segall, Schult manuscript and '
                     'jjn.py code for more details', fontsize=12)
        helpwin.show()
    ...
    def update_plot(val):
        ...
        output:  recalculates and replots the AP and current data.
                 Returns None and does nothing if the nonlocal variable
                 izh_button_clicked is True.
        errors:  none.'''

        if izh_button_clicked:
            return None
    ...
    def update_values(val, widtype, param_set, el):
        ...
        output:  if a slider value has changed, updates the value of the
                 ...
                 the figure.  If the nonlocal izh_button_clicked variable
                 is True, returns None and does nothing.
        errors:  none.'''
